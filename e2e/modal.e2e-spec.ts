...
describe('Modal', () => {
  ...
  describe('content with background', () => {
    it('should match previous modal screenshot with background', (done) => {
      SkyHostBrowser.setWindowBreakpoint('lg');
      element(by.css('.sky-btn-primary')).click();
      expect('.sky-modal-host-backdrop').toMatchBaselineScreenshot(done);
    });
  });
});
